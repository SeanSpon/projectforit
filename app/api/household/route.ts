...
export async function GET(){try{return response(await readHouse());}catch{return response({error:"The database is unavailable. Try again shortly."},503);}}
export async function POST(request:Request){
  if(request.headers.get("sec-fetch-site")==="cross-site")return response({error:"Cross-site requests are not allowed."},403);
  ...
  try{
    ...
    let next;try{next=applyAction(snapshot.state,parsed.data.action);}catch(e){return response({error:e instanceof Error?e.message:"Invalid action."},400);}
